...
markerTest <- getMarkerFeatures(
        ...
        useGroups = "Group_B",
        bgdGroups = c("Group_A", "Group_C", "Group_D")
)

sigMarkers <- getMarkers(markerTest, cutOff = "FDR <= 0.05 & abs(Log2FC) >= 0.25") # get signif

filename <- "/storage/home/mfisher42/scProjects/CD_Subra/Subset_Specific_DEGs_DARs/All_DARs_04112023/T_Cells/GroupB_DARs.csv"
write.csv(sigMarkers$Group_B, file = filename)
