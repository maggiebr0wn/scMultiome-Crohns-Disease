...
cluster_files <- list.files(path = "/storage/home/mfisher42/scProjects/CD_Subra/ArchR_Project/Multiome_07_2022/ArchR_07_2022/Filtered-Final-projIBD-ATACandGEX-07-2022/CLUSTS_UMAPS/Harmony_Combo",
                    pattern = ".csv")

# only keep cluster_files
cluster_files <- str_subset(cluster_files, "Clusts", negate = FALSE)
cluster_files <- str_subset(cluster_files, "Harmonyv2", negate = FALSE)
clusters <- gsub('.{4}$', '', cluster_files)

umap_files <- list.files(path = "/storage/home/mfisher42/scProjects/CD_Subra/ArchR_Project/Multiome_07_2022/ArchR_07_2022/Filtered-Final-projIBD-ATACandGEX-07-2022/CLUSTS_UMAPS/UMAP_Coords/Harmony_Combo",
                            pattern = ".csv")
umap_files <- str_subset(umap_files, "Harmonyv2", negate = FALSE)
umaps <- gsub('.{4}$', '', umap_files)

names(umaps) <- clusters

### Loop through each UMAP/Cluster set for plots: ###
setwd("/storage/home/mfisher42/scProjects/CD_Subra/ArchR_Project/Multiome_07_2022/ArchR_07_2022/Filtered-Final-projIBD-ATACandGEX-07-2022/CLUSTS_UMAPS/Harmony_Combo/RandIndex/Harmonyv2/Plots")
...
for (umap_set in umaps){
  ...
  lsi <- paste(
    str_split_fixed(umap_set, "_", 7)[,3],
    str_split_fixed(umap_set, "_", 7)[,4],
    str_split_fixed(umap_set, "_", 7)[,5],
    str_split_fixed(umap_set, "_", 7)[,6],
    sep = "_"
  )
